...
const EmbeddingSpaceScene = dynamic(
  () => import("@/components/landing/embedding-space-scene").then((mod) => mod.EmbeddingSpaceScene),
  { ssr: false }
);

const steps = [
  { icon: Upload, title: "Upload", desc: "Drop PDFs, DOCX, MD, or TXT." },
  { icon: Scissors, title: "Parse + Chunk", desc: "Split into semantic chunks." },
  { icon: Database, title: "Embed + Store", desc: "Generate vectors and persist." },
  { icon: Search, title: "Retrieve + Generate", desc: "Answer with citations." },
];
    ...
    <AuroraBackground className="relative min-h-screen">
      ...
      <header className="sticky top-0 z-50 border-b border-transparent backdrop-blur supports-[backdrop-filter]:bg-[hsl(var(--color-bg))/0.45]">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <Link href="/" className="text-sm font-semibold text-[hsl(var(--color-text-primary))]">
            claude-rag-template
          </Link>
          ...
          <MagneticButton href="/chat" variant="secondary">
            Try the demo
          </MagneticButton>
        </div>
      </header>
      ...
      <main>
        <section className="relative min-h-[100svh] px-6 py-24">
          ...
          <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2">
            <div className="space-y-6">
              ...
              <h1 className="font-display text-5xl leading-[0.95] tracking-tight text-[hsl(var(--color-text-primary))] md:text-7xl">
                ...
                <br />
                in <span className="bg-gradient-to-r from-[hsl(var(--color-accent))] to-[hsl(var(--color-text-primary))] bg-clip-text text-transparent">minutes</span>, not months.
              </h1>
              <p className="max-w-2xl text-lg text-[hsl(var(--color-text-secondary))]">
                Document upload, embeddings, semantic search, streaming citations — done right. No LangChain bloat.
              </p>
              ...
              <p className="text-sm text-[hsl(var(--color-text-tertiary))]">No signup required · 5-second setup</p>
            </div>
        ...
        <section id="how" className="mx-auto max-w-7xl px-6 py-24">
          <RevealOnScroll>
            <h2 className="font-display text-4xl text-[hsl(var(--color-text-primary))]">From document to answer in 4 steps.</h2>
          </RevealOnScroll>
          <div className="mt-10 grid gap-4 md:grid-cols-4">
            {steps.map((step, i) => (
              <RevealOnScroll key={step.title} delay={i * 0.05}>
                <GlowCard className="relative rounded-2xl p-5">
                  {i < steps.length - 1 ? <BorderBeam className="opacity-35" /> : null}
                  <step.icon className="mb-3 size-5 text-[hsl(var(--color-accent))]" />
                  <h3 className="text-lg text-[hsl(var(--color-text-primary))]">{step.title}</h3>
                  <p className="mt-1 text-sm text-[hsl(var(--color-text-secondary))]">{step.desc}</p>
                </GlowCard>
              </RevealOnScroll>
            ))}
          </div>
        ...
        <section id="features" className="mx-auto max-w-7xl px-6 py-24">
          <div className="grid gap-4 md:grid-cols-4">
            <GlowCard className="md:col-span-2 md:row-span-2 rounded-2xl p-6">
              <h3 className="text-xl text-[hsl(var(--color-text-primary))]">Streaming citations</h3>
              <p className="mt-2 text-sm text-[hsl(var(--color-text-secondary))]">Responses stream in with source references.</p>
            </GlowCard>
        ...
        <section id="demo" className="mx-auto max-w-7xl px-6 py-24">
          <h2 className="font-display text-4xl text-[hsl(var(--color-text-primary))]">RAG in 10 lines, no LangChain.</h2>
          <div className="mt-8 grid gap-6 lg:grid-cols-2">
            <GlowCard className="rounded-2xl p-5">
              <ShimmerText className="text-sm">app/api/chat/route.ts</ShimmerText>
...
return streamText({ model: nvidia('deepseek-v4-pro'), prompt })`}</pre>
            ...
            <GlowCard className="rounded-2xl p-5">
              <p className="text-sm text-[hsl(var(--color-text-secondary))]">Without wrapper stack: 200+ lines, 4 deps</p>
              <p className="mt-3 text-lg text-[hsl(var(--color-text-primary))]">With this template: 10 lines, 0 wrapper libs</p>
            </GlowCard>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-6 py-16">
          <GlowCard className="rounded-2xl p-6">
            <h3 className="text-xl text-[hsl(var(--color-text-primary))]">Live demo teaser</h3>
            <p className="mt-2 text-sm text-[hsl(var(--color-text-secondary))]">Interactive read-only preview with seeded data.</p>
            <Link href="/chat" className="mt-4 inline-flex text-[hsl(var(--color-accent))]">
              Try the full demo →
            </Link>
          </GlowCard>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-6 py-12">
          <div className="grid gap-4 rounded-2xl border border-[hsl(var(--color-border-strong))] bg-[hsl(var(--color-bg-overlay))/0.7] p-6 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              <AnimatedNumber value={1536} className="text-2xl text-[hsl(var(--color-text-primary))]" />
              <p className="text-xs text-[hsl(var(--color-text-secondary))]">dim vectors</p>
            </div>
            <div>
              <AnimatedNumber value={50} className="text-2xl text-[hsl(var(--color-text-primary))]" />
              <p className="text-xs text-[hsl(var(--color-text-secondary))]">&lt;50ms retrieval</p>
            </div>
            <div>
              <AnimatedNumber value={0} className="text-2xl text-[hsl(var(--color-text-primary))]" />
              <p className="text-xs text-[hsl(var(--color-text-secondary))]">LangChain deps</p>
            </div>
            <div>
              <AnimatedNumber value={100} className="text-2xl text-[hsl(var(--color-text-primary))]" />
              <p className="text-xs text-[hsl(var(--color-text-secondary))]">% open source</p>
            </div>
          </div>
        </section>

        <section className="mx-auto max-w-4xl px-6 py-24">
          <h2 className="font-display text-4xl text-[hsl(var(--color-text-primary))]">FAQ</h2>
          <Accordion className="mt-8 space-y-3">
            <AccordionItem value="q1" className="rounded-xl border border-[hsl(var(--color-border-strong))] bg-[hsl(var(--color-bg-overlay))/0.7] px-4">
              <AccordionTrigger>Does this include auth and citations?</AccordionTrigger>
              <AccordionContent>Yes. The template includes authenticated upload + chat with source citations.</AccordionContent>
            </AccordionItem>
            <AccordionItem value="q2" className="rounded-xl border border-[hsl(var(--color-border-strong))] bg-[hsl(var(--color-bg-overlay))/0.7] px-4">
              <AccordionTrigger>Can I deploy it quickly?</AccordionTrigger>
              <AccordionContent>Yes. Add env vars and deploy to Vercel in minutes.</AccordionContent>
            </AccordionItem>
          </Accordion>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-6 py-24">
          <GlowCard className="rounded-2xl p-10 text-center">
            <h2 className="font-display text-4xl text-[hsl(var(--color-text-primary))]">Start building your RAG product today</h2>
            ...
            <p className="mt-4 text-xs text-[hsl(var(--color-text-tertiary))]">GitHub stars badge placeholder</p>
          </GlowCard>
        </section>
      </main>
